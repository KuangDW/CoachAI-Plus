...
const Evaluation = () => {
  const { files, setFiles } = useShotStore();
  const { n, setN } = useShotStore();
  const { loading, setLoading } = useShotStore();
  const { data, setData } = useShotStore();
  const { index, setIndex } = useShotStore();
