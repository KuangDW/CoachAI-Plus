...
def bad_net(
    ...
    input_player_area = tf.keras.Input(
        shape=shot_sequence_shape[0], name="Player_area_input"
    )
    input_opponent_area = tf.keras.Input(
        shape=shot_sequence_shape[0], name="Opponent_area_input"
    )

    if embed_area_size is not None:
        area_embedding = tf.keras.layers.Embedding(
            input_dim=embed_area_size,
            output_dim=10,
            mask_zero=True,
            name="Area_embedding",
        )
    ...
    if embed_types_size:
        input_shot_types = tf.keras.Input(
            shape=shot_sequence_shape[0], name="Shot_types_input"
        )
        layer_embedding = tf.keras.layers.Embedding(
            input_dim=embed_types_size,
            output_dim=15,
            mask_zero=True,
            name="Shot_types_embedding",
        )
        shot_mu_embedding = tf.keras.layers.Embedding(
            input_dim=embed_types_size,
            output_dim=15,
            mask_zero=True,
            name="Time_influence_occurrence",
        )
        shot_theta_embedding = tf.keras.layers.Embedding(
            input_dim=embed_types_size,
            output_dim=15,
            mask_zero=True,
            name="Time_influence_shot",
        )

        input_time_proportion = tf.keras.Input(
            shape=shot_sequence_shape[0], name="Time_proportion_input"
        )
        ...
        layer_concat_embedding = tf.keras.layers.Concatenate(
            name="Shots_features_merging"
        )
    else:
    ...
    layer_cnn = custom_layers.StaggeredConv1D(
        name="Local_pattern_extration", **cnn_kwargs
    )
    ...
    layer_rnn = tf.keras.layers.Bidirectional(
        tf.keras.layers.GRU(return_sequences=True, **rnn_kwargs),
    ...
    input_shots_concat_area = shots_concat_areas(
        [embeded_hit_area, embeded_player_area, embeded_opponent_area, input_shots]
    )
    ...
    if embed_types_size is not None:
        ...
        time_mu_proportion = time_multiplication(
            [
                embeded_shot_mu,
                tf.tile(tf.expand_dims(input_time_proportion, -1), [1, 1, 15]),
            ]
        )
    ...
    # transformer family
    pos_embed_seq = layer_pos_embed(pattern_sequence)
    # transformer
    encoder_result, attention_weights_list = get_encoders(
        input_layer=pos_embed_seq, **transformer_kwargs
    )
    ...
    model_attention = tf.keras.Model(inputs=inputs, outputs=attention_weights_list)
...
def rnn(
    ...
    rnns = {"gru": tf.keras.layers.GRU, "lstm": tf.keras.layers.LSTM}
    # Layers
    input_shots = tf.keras.Input(shape=shot_sequence_shape, name="Shots_input")
    layer_masking = tf.keras.layers.Masking(name="Sequence_masking")
...
def deepmoji(
    ...
    layer_concat_input_rnn = tf.keras.layers.Concatenate(name="Input_rnn_merging")
    layer_attention = keras_self_attention.SeqWeightedAttention(
        return_attention=True, **attention_kwargs
    )
    if rally_info_shape is not None:
    ...
    input_states = layer_concat_input_rnn([masked_sequence, hidden_states])
...
def prosenet(
    ...
    model_predict = ProSeNet(
        input_shape=shot_sequence_shape,
        nclasses=2,
        rnn_args=rnn_kwargs,
        **prosenet_kwargs
    )
