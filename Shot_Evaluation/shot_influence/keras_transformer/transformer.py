...
def _wrap_layer(name, input_layer, build_func, dropout_rate=0.0, trainable=True):
    ...
    if isinstance(build_output, list):  # modified
        build_output, attention_weights = build_output
    else:
    ...
    return normal_layer, attention_weights  # modified
...
def get_encoder_component(
    ...
    attention_layer, attention_weights = _wrap_layer(  # modified
        name=attention_name,
    ...
    feed_forward_layer, _ = _wrap_layer(  # modified
    ...
    return feed_forward_layer, attention_weights  # modified


def get_decoder_component(
    name,
    input_layer,
    encoded_layer,
    head_num,
    ...
    feed_forward_name = "%s-FeedForward" % name
    self_attention_layer, self_attention_weights = _wrap_layer(  # modified
    ...
    query_attention_layer, query_attention_weights = _wrap_layer(  # modified
...
def get_encoders(
    ...
    attention_weights_list = []  # modified
    for i in range(encoder_num):
        last_layer, attention_weights = get_encoder_component(  # modified
            name="Encoder-%d" % (i + 1),
        ...
        attention_weights_list.append(attention_weights)  # modified
    return last_layer, attention_weights_list  # modified
...
def get_decoders(
    ...
    """
    last_layer = input_layer
    self_attention_weights_list = []  # modified
    query_attention_weights_list = []
    for i in range(decoder_num):
        last_layer, (self_attention_weights, query_attention_weights) = (
            get_decoder_component(  # modified
                name="Decoder-%d" % (i + 1),
                input_layer=last_layer,
        ...
        self_attention_weights_list.append(self_attention_weights)  # modified
        query_attention_weights_list.append(query_attention_weights)
    return last_layer, self_attention_weights_list, query_attention_weights_list
...
def get_model(
    ...
    return keras.models.Model(
        inputs=[encoder_input, decoder_input], outputs=output_layer
    )
    ...
    while len(list(filter(lambda x: x is None, outputs))) > 0:
        ...
        for i in range(len(predicts)):
            if top_k == 1:
                ...
                probs = probs[:top_k]
                indices, probs = list(map(lambda x: x[1], probs)), list(
                    map(lambda x: x[0], probs)
                )
            ...
            if (
                ...
                or (max_len is not None and output_len >= max_len)
                or _get_max_suffix_repeat_times(
                    decoder_inputs[index_map[i]], max_repeat * max_repeat_block
                )
                >= max_repeat
            ):
