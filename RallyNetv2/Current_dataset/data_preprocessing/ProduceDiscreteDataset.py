...
data_path = 'data_tables/first_process_continuous.csv' 
data = pd.read_csv(data_path)


# 定義球沒有打過去的 landing area = 33
...
def coord2area(point_x, point_y):
    ...
    hit_area = mistake_landing_area # 表示沒有打到對面去 無動作
...
for index, row in data.iterrows():  
    area = coord2area(data.loc[index, 'player_location_x'], data.loc[index, 'player_location_y'])
    data.loc[index, 'player_location_area'] = area
    area = coord2area(data.loc[index, 'hit_x'], data.loc[index, 'hit_y'])
    data.loc[index, 'hit_area'] = area
    area = coord2area(data.loc[index, 'landing_x'], data.loc[index, 'landing_y'])
    data.loc[index, 'landing_area'] = area
    area = coord2area(data.loc[index, 'opponent_location_x'], data.loc[index, 'opponent_location_y'])
    data.loc[index, 'opponent_location_area'] = area
    area = coord2area(data.loc[index, 'moving_x'], data.loc[index, 'moving_y'])
    data.loc[index, 'player_move_area'] = area

data.to_csv("data_tables/second_process_discrete.csv", encoding = 'utf-8',index = False)
