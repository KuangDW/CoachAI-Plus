    ...
    for player_rally, opponent_rally in zip(player_data, opponent_data):
        ...
        if player_len > 0:
            ...
            for i in range(max_len):
                """ only training for serve """
                if player_rally[i][20] in [1,2] and player_rally[i][12] == 0:
                    if i < player_len:
                        merged_rally.append(player_rally[i])
                        merged_data.append(merged_rally)
                        shot_type_set.append(player_rally[i][20])
                break            
...
class MLP(nn.Module):
    def __init__(self, input_size, hidden_size, player_id_len, shot_type_len):
        ...
        self.predict_shot_type = nn.Linear(hidden_size, shot_type_len + 1, bias=False)
